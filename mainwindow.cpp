    ...
    ui(new Ui::MainWindow)
{
    ...
    this->setWindowTitle(QStringLiteral("OCV_分流换框"));
...
//    m_pLogView->show();
    ...
    connect(m_pMainControl, &MainControl::sig_ReadPLCData_1,
            this, &MainWindow::slot_ReadPLCData_1);

    connect(m_pMainControl, &MainControl::sig_ReadPLCData_2,
            this, &MainWindow::slot_ReadPLCData_2);

    connect(m_pMainControl, &MainControl::sig_ReadPLCData_3,
            this, &MainWindow::slot_ReadPLCData_3);
...
void MainWindow::slot_SportCardConnectState(int n_ID, bool b_State)
{
    ...
    if(b_State){
        m_listSportCardStateLaber.at(3)->setText(QStringLiteral("连接"));
        m_listSportCardStateLaber.at(3)->setStyleSheet(Connect_Style);
        m_listSportCardStateLaber.at(4)->setText(QStringLiteral("连接"));
        m_listSportCardStateLaber.at(4)->setStyleSheet(Connect_Style);
    }
    else{
        m_listSportCardStateLaber.at(3)->setText(QStringLiteral("断开"));
        m_listSportCardStateLaber.at(3)->setStyleSheet(Disconnect_Style);
        m_listSportCardStateLaber.at(4)->setText(QStringLiteral("断开"));
        m_listSportCardStateLaber.at(4)->setStyleSheet(Disconnect_Style);
    }
}

void MainWindow::slot_PLCConnectState(int n_Station, bool b_Connect)
{
    if(n_Station == 1){
        for(int i = 1; i < 6; i++){
            if(b_Connect){
...
void MainWindow::slot_ReadPLCData_1(PLC_DATAAREA_1 PLCData)
{
    unsigned char ucCurrentData[20] = {};

    memset(ucCurrentData, 0, 20);
    memcpy(ucCurrentData, &PLCData, 10);
...
void MainWindow::slot_ReadPLCData_2(PLC_DATAAREA_2 PLCData)
{
    unsigned char ucCurrentData[4] = {};

    memset(ucCurrentData, 0, 4);
    memcpy(ucCurrentData, &PLCData, 2);
...
void MainWindow::slot_ReadPLCData_3(PLC_DATAAREA_3 PLCData)
{
    unsigned char ucCurrentData[16] = {};

    memset(ucCurrentData, 0, 16);
    memcpy(ucCurrentData, &PLCData, 8);
